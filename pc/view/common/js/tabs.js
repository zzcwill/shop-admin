var comn, e, getWidth, menuItemClick, tabActive, tabClose, tabCloseAll, tabCloseOther, tabLeft, tabRefresh, tabRight, tabSwitch, toUrl, urlArr = [], currentParentIndex;
...
menuItemClick = function() {
  ...
  if (add) {
    ...
    menuTab = "<a href = 'javascript:;' current-index='"+currentIndex+"' parent-index='"+parentIndex+"' data-index='"+ dataIndex +"' class = 'active J_menuTab' data-id= '" + url + "'> " + title + " <i class = 'fa fa-times-circle'></i> </a>";
...
function closeTab(url) { 
	tabClose.call($(".J_menuTabs .active.J_menuTab i"),url); 
...
tabClose = function(path) {
  ...
  if (_curMenuTab.hasClass("active")) {
    if(path){
      var menuTabIndex = -1;
      var menuTab = _curMenuTab.parent(".page-tabs-content").find(".J_menuTab")
      for(var i = 0; i < menuTab.length; i++){
        // console.log(menuTab.eq(i).attr("data-id"), menuTab.eq(i).attr("data-id") == path, path)
        if(menuTab.eq(i).attr("data-id") == path){
          menuTabIndex = i
          break; 
        }
      }
      if(menuTabIndex != -1){
        lastUrl = _curMenuTab.parent(".page-tabs-content").find(".J_menuTab").eq(menuTabIndex).addClass("active").data("id");
        $(".J_mainContent .J_iframe").each(function() {
...
tabSwitch = function() {
  ...
  if (!$(this).hasClass("active")) {
    url = $(this).data("id");
    var currentIndex = Number($(this).attr("current-index"))
    var parentIndex = Number($(this).attr("parent-index"))
    //console.log(currentParentIndex,parentIndex)
    $(".J_menuItem").removeClass("select")
    if(currentParentIndex != parentIndex){
      currentParentIndex = parentIndex
      $("#side-menu").children().eq(parentIndex).children().eq(0).click()
      $("#side-menu").children().eq(parentIndex).find("ul").children().eq(currentIndex).find(".J_menuItem").eq(0).addClass("select")
    }else{
      $("#side-menu").children().eq(parentIndex).find("ul").children().eq(currentIndex).find(".J_menuItem").eq(0).addClass("select")
    }
    $(".J_mainContent .J_iframe").each(function() {
